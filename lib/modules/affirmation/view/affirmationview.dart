...
class AffirmationView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Demo data to show when API fails or is loading
    ...
    return ThemedScaffold(
      body: SafeArea(
        child: Padding(
          ...
          child: Obx(() {
            // Determine if we show the real data or the skeleton/demo state
            final bool isSkeleton = controller.isLoading.value || controller.affirmationList.isEmpty;
            
            // Fallback to demo item if list is empty
            ...
            return Column(
              children: [
                ...
                // --- Internet Connection / Error Indicator ---
                ...
                // --- Main Card (Skeleton Opacity applied here) ---
                Opacity(
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      // Navigation Arrows (Disabled in Skeleton mode)
                      Positioned(
                        left: 8,
                        child: IconButton(
                          icon: CircleAvatar(
                            ...
                            child: Icon(
                              Icons.chevron_left, 
                              color: isSkeleton ? Colors.grey : Colors.black
                            )
                          ),
                          onPressed: isSkeleton ? null : () => controller.previousAffirmation(),
                        ),
                      ),
                      Positioned(
                        right: 8,
                        child: IconButton(
                          ...
                          onPressed: isSkeleton ? null : () => controller.nextAffirmation(),
                        ),
                      ),
                ...
                if (controller.hasError.value && !controller.isLoading.value)
                  TextButton.icon(
                    onPressed: () => controller.getAffirmations(),
                  ...
                  children: [
                    Expanded(
                      child: _buildBtn(
                        onTap: isSkeleton 
                            ? () {} 
                            : () => controller.toggleFavorite(currentItem.id, controller.currentIndex.value),
